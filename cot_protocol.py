...
class CoTEvent:
    ...
    def to_xml(self) -> str:
        ...
        # originated from a Meshtastic node.  The atak-forwarder plugin uses the
        # same element; by including it in every Meshtastic node CoT generated by
        # LPU5 we ensure that TAK clients preserve the element when they
        # re-broadcast the event.  The <meshtastic> presence is then detected by
        # detail_has_meshtastic() in _process_incoming_cot (api.py) and
        # cot_to_marker(), so the node is correctly identified even if the TAK
        # client normalises the CoT type code (e.g. strips "-E-S-U-M").
        if self.is_meshtastic_node and self.callsign:
